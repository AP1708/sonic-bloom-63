...
import { LyricsPane, QueueList } from "./side-panel";
            ...
            {(["queue", "lyrics"] as const).map((value) => (
              <button
                ...
                className={cn(
                  ...
                  tab === value ? "bg-surface-raised text-primary" : "text-muted-foreground hover:text-foreground",
                )}
              >
                {value === "queue" ? "Up next" : "Lyrics"}
              </button>
            ))}
          </div>
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
            {tab === "queue" ? <QueueList /> : <LyricsPane />}
          </div>
        </aside>
      </div>
    </div>
